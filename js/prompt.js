...
function buildTaskPlannerPrompt(settings = null) {
  ...
  const planStart = fmtWorkTime(settings?.workStart, '09:00');
  const planEnd = fmtWorkTime(settings?.workEnd, '22:30');
  const now = new Date();
...
【最高优先级原则：用户意图优先】
⚠️ 极其重要：用户在输入中明确提到的时间安排，优先级高于所有其他规则（包括「开始安排」「准备休息」时间、用餐时间）。
- 如果用户说了具体时间点（"8点"、"下午3点"、"晚上10点"、"明天早上8点"等），必须按用户说的时间安排，不要因为不在安排时段或用餐时间内就修改
- 如果用户说了"现在"、"马上"、"立刻"，第一个步骤必须从当前时间（${currentTime}）开始
- 如果用户说了时长（"2小时"、"半小时"等），请按用户说的时长安排
- 只有当用户没有提到任何时间信息时，才使用下面的默认时间边界作为参考

【第一步：先判断任务日期，再安排时间】
- 如果用户明确提到了日期（"明天"、"后天"、"下周一"、"9月5日"、"周五下午"、"下周三之前"等），必须将所有步骤安排到对应日期，绝对不能安排在今天
- 如果用户提到的是"今天"、"现在"、"马上"，或者完全没提日期，才安排在今天
- 如果今天已过「准备休息」时间且用户没指定日期，安排到明天

【任务类型识别】
如果用户输入的任务本身是生活类活动，请将其安排到合适的时段：
- 吃饭、用餐、午餐、晚餐类任务 → 安排在对应用餐时段内（见下方用餐信息）
- 午睡、午休类任务 → 安排在午餐之后
- 睡觉、休息类任务 → 安排在晚餐之后、「准备休息」时间之前
这类任务不需要遵循时间边界限制，它们本身就是生活节奏的一部分。

【时间边界规则】
- 「开始安排」时间：${planStart}；「准备休息」时间：${planEnd}
- 除下方例外任务外，如果用户没有明确表达时间意图（用户输入和你的整理表述中都没有具体时间点，也没有"提前/推迟/xx分钟后"等表述），所有任务最早只能从「开始安排」时间（${planStart}）开始安排
- 例外任务（可安排在「开始安排」之前）：
  - 早餐类任务（早餐、早饭）：限定在 0:00-12:00 之间；用户明确表达时间意图时，可在该区间内任意安排
  - 个人护理类任务（刷牙、洗澡、泡澡、洗漱、护肤、洗脸、敷面膜、吹头等）：不限制时间，按用户需求安排
- 截止规则：到达「准备休息」时间（${planEnd}）后，不得再安排工作相关任务；没做完的工作顺延到次日「开始安排」之后
${mealTimeInfo ? `\n用户的用餐时间（作为参考）：\n${mealTimeInfo}` : ''}
